...
def create_pdf(name, age, contact, city, category, scores, code, interest_results):
    ...
    if interest_results:
        ...
        story.append(Paragraph(
            "6. Карта интересов подростка" if category == "Подросток" else "6. Карта интересов взрослого",
            styles["SectionTitle"]
        ))

        if category == "Подросток":
            intro_text = (
                "Ниже представлены наиболее выраженные сферы интересов. "
                "Этот блок помогает увидеть направления, которые вызывают "
                "наибольший отклик и могут быть полезны при выборе обучения и профессии."
            )
        else:
            intro_text = (
                "Ниже представлены наиболее выраженные сферы интересов. "
                "Этот блок помогает лучше понять направления, которые могут подойти "
                "для профессионального развития, переориентации или более глубокого изучения."
            )
        ...
        interest_table_data = [["Сфера интересов", "Балл", "Уровень"]]
        for item in top_interests:
            interest_table_data.append([
                item["name"],
                str(item["score"]),
                item["level"]
            ])
        ...
        interest_table = Table(
            interest_table_data,
            colWidths=[8.6 * cm, 2.2 * cm, 4.2 * cm]
        )
        ...
        high_interests = [item["name"] for item in interest_results if item["level"] == "Высокий интерес"][:5]
        ...
        if high_interests:
            if category == "Подросток":
                interest_summary = (
                        ...
                        ", ".join(high_interests) +
                        ". Эти направления особенно важно учитывать при выборе профиля обучения, "
                        "дополнительных занятий и дальнейшем профессиональном самоопределении."
                )
            else:
                interest_summary = (
                        "Наиболее выраженные интересы: " +
                        ", ".join(high_interests) +
                        ". Эти направления стоит рассматривать как опорные точки для "
                        "профессионального развития, смены траектории или уточнения карьерных целей."
                )
        else:
            if category == "Подросток":
                interest_summary = (
                    "Ярко выраженных интересов не выявлено. Это может означать, что сферы интересов "
                    "пока находятся в стадии формирования, и полезно продолжать исследовать разные направления."
                )
            else:
                interest_summary = (
                    "Ярко выраженных интересов не выявлено. Это может говорить о широком диапазоне интересов "
                    "или о необходимости дополнительного анализа профессиональных предпочтений."
                )

        story.append(Paragraph(interest_summary, styles["Body"]))
        story.append(PageBreak())
    ...
    # ----------------------------
    story.append(CondPageBreak(4 * cm))
    story.append(Paragraph("7. Практические рекомендации", styles["SectionTitle"]))
    ...
    if category == "Подросток":
        ...
        rec_text_1 = (
            "При выборе следующего профессионального шага рекомендуется учитывать не только ведущие типы интересов, "
            "но и уже накопленный опыт, сильные навыки, жизненные обстоятельства и реалистичные варианты развития."
        )
        rec_text_2 = (
            f"На основании результатов теста стоит обратить внимание на следующие направления: "
            f"<b>{', '.join(recommended_fields)}</b>."
        )
        rec_text_3 = (
            "Полезно проверить выводы отчёта через практику: короткие учебные программы, профессиональные пробы, "
            "тестовые задачи, стажировки, консультации со специалистами и анализ смежных карьерных маршрутов."
        )
        rec_text_4 = (
            "Если вопрос профессионального выбора или смены деятельности остаётся открытым, "
            "отчёт лучше рассматривать как основу для дальнейшей карьерной консультации, "
            "профпереподготовки или уточнения новой траектории развития."
        )
    ...
    if category == "Подросток":
        summary_text = (
            ...
            f"<b>{TYPE_LABELS[top3[0]]}</b>, <b>{TYPE_LABELS[top3[1]]}</b> и <b>{TYPE_LABELS[top3[2]]}</b>. "
            f"Именно сочетание этих особенностей полезно учитывать при выборе профиля обучения, "
            f"дополнительных занятий и первых шагов в профессиональном самоопределении."
        )
    else:
        summary_text = (
            f"Профиль <b>{code}</b> указывает на то, что наиболее естественная профессиональная реализация "
            f"может происходить в тех сферах, где человек сможет использовать качества, свойственные типам "
            f"<b>{TYPE_LABELS[top3[0]]}</b>, <b>{TYPE_LABELS[top3[1]]}</b> и <b>{TYPE_LABELS[top3[2]]}</b>. "
            f"Именно сочетание этих особенностей важно учитывать при выборе следующего карьерного шага, "
            f"уточнении профессионального вектора или смене направления деятельности."
        )
